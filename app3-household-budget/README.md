...
WebSocket / リアルタイム通信・グラフ可視化・高度な TypeScript 型パターンを組み合わせ、「設計力のある TypeScript エンジニア」であることを示すアプリケーション。
App1・App2 と異なる技術領域（リアルタイム・データビジュアライゼーション・E2E テスト）をカバーし、ポートフォリオとしての幅の広さをアピールする。
...
|------|-----------|----------|------------------------|
| Next.js App Router | 14 | App2 で習得済。短期間で高品質なアプリを作れる実証 | 「習得した技術を素早く応用できる」 |
| TypeScript | 5（strict） | 全3アプリ共通の strict 設定。一貫した品質基準 | 「プロジェクト横断で型安全を維持する習慣」 |
| Supabase Realtime | - | WebSocket をゼロ設定で使える。型付きクライアントを自動生成できる | 「リアルタイム機能の設計・実装経験」 |
| Recharts | 2 | TypeScript 型定義が完全。React との統合が自然で学習コストが低い | 「データビジュアライゼーションの実装経験」 |
| Playwright | 1 | E2E テストは採用企業が最も重視する品質指標の一つ。CI 統合も容易 | 「テスト戦略の設計・実装経験（Unit〜E2E）」 |
| date-fns | 3 | 日付操作の型安全ライブラリ。moment.js との比較・Tree-shaking の説明ができる | 「ライブラリ選定の判断軸を持っている」 |
| Tailwind CSS | 3 | App1・App2 から継続。統一した UI 実装 | 「スタック一貫性の重要性を理解している」 |
...
| 30 | 全3アプリの README 最終版・ポートフォリオサイト更新 | ポートフォリオ完成 |
...
1. **リアルタイム設計の意思決定**: WebSocket をフルスクラッチで実装せず Supabase Realtime を選んだ理由（開発速度・型安全性・インフラ管理コスト削減）を説明できる
2. **高度な TypeScript パターンの活用**: Mapped Types・Type Predicates・`satisfies` 演算子を実際のコードで示せる
3. **テスト戦略の全体像**: Unit（Vitest）→ Integration → E2E（Playwright）の3層構造を App1〜App3 を通じて実践したことを示す
